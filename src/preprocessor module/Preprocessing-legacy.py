            ...
            for mat_file in os.listdir(eeg_folder_path):
                if mat_file.endswith('.mat'):
                    ...
                    try:
                        ...
                        if 'cnt' in mat:
                            cnt_group = mat['cnt']
                            print(f"Available keys in 'cnt' group: {list(cnt_group.keys())}")
                            if 'EEG' in cnt_group:
                                cnt_data = cnt_group['EEG'][:]
                                fs = cnt_group.attrs['fs'] if 'fs' in cnt_group.attrs else 1000
                                print(f"Preprocessing cnt data with shape {cnt_data.shape}...")
                                cnt_data_preprocessed = preprocess_data(cnt_data, fs)

                                # Move the preprocessed data to the new folder
                                new_cnt_file_path = os.path.join(preprocessed_dataset_dir, f"{folder}_cnt.mat")
                                with h5py.File(new_cnt_file_path, 'w') as new_mat:
                                    new_mat.create_dataset('cnt', data=cnt_data_preprocessed)
                            ...
                            try:
                                for key in mrk_group.keys():
                                    ...
                                    if isinstance(item, h5py.Dataset):
                                        print(f"Dataset {key} found with shape {item.shape}")
                                        data = item[:]
                                        print(f"Data from {key}: {data[:5]}")
                                    else:
                                        print(f"Group {key} found, not a dataset.")

                                new_mrk_file_path = os.path.join(preprocessed_dataset_dir, f"{folder}_mrk.mat")
                                with h5py.File(new_mrk_file_path, 'w') as new_mat:
                                    for key in mrk_group.keys():
                                        item = mrk_group[key]
                                        if isinstance(item, h5py.Dataset):
                                            new_mat.create_dataset(key, data=item[:])
                                print(f"Saved processed mrk data to {new_mrk_file_path}")
                            except KeyError as e:
                                print(f"Error: Key {e} not found in 'mrk' group in {mat_file_path}.")
                            except Exception as e:
                                print(f"Error processing 'mrk' group in {mat_file_path}: {e}")
