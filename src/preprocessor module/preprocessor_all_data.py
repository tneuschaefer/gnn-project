...
def extract_mrk_info(mrk_mat):
    ...
    if isinstance(mrk_mat, dict):
        if "mrk" in mrk_mat:
            ...
            keys = [k for k in mrk_mat.keys() if isinstance(k, str) and k.lower().startswith("mrk")]
            if keys:
        ...
        try:
            ...
            try:
                mrk_event = np.array(mrk_struct.event if hasattr(mrk_struct, 'event') else mrk_struct["event"])
            except Exception:
                mrk_event = None
        ...
        if "mrk" in top_keys:
            ...
            keys = [k for k in top_keys if isinstance(k, str) and k.lower().startswith("mrk")]
            if keys:
...
def normalize_epochs(epochs):
    ...
    epochs_normalized = mne.EpochsArray(norm_data, info=epochs.info, events=epochs.events)
...
def preprocess_subject(subject_folder, raw_root="./data/raw/", output_root="./data/preprocessed/"):
    ...
    if not all([f == fs_list[0] for f in fs_list]):
        print(f"Warning: Sampling frequencies differ in subject {subject_folder}. Using the first one.")
    fs = fs_list[0]
